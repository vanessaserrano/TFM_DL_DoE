...
app_server<-function(input,output,session){
  ...
  proxy=dataTableProxy("tabla_factores")
  replaceData(proxy,"tabla_factores")
  ...
  tabla_niv=reactive({
    ...
    if(tabla_factores[,2]==2){
      nombre_columna_niv<-c("Nivel 1","Nivel 2")
      d3=data.frame(matrix(nrow=input$num_factores,ncol=2))
      colnames(d3)<-nombre_columna_niv
      d3<-as.data.frame(d3)}
  })
  
  output$tabla_niveles<-renderDataTable(tabla_niv(),editable=TRUE)
  ...
  observeEvent(input$calcmatriz,{
    output$diseno_sugerido<-renderText("Diseño sugerido, debe depender de alguna variable de calc")
  })
